defmodule ElixirDataScience.BLSConformanceReportTest do
  ...
  test "builds a label-independent comparison report without available source values", %{
    ...
    assert report["value_handling"]["preliminary"]["aligned_observation_count"] == 4
  ...
  test "rejects a registered request presented as the anonymous experiment", %{
    analysis: analysis,
    config: config,
    dataset: dataset
  } do
    registered_dataset = Map.put(dataset, :request_mode, :registered)

    assert {:error, {:request_configuration_mismatch, details}} =
             BLSConformanceReport.build(registered_dataset, analysis, config)

    assert details.actual_request_mode == :registered
    assert details.expected_request_mode == :anonymous
  end

  test "keeps comparison profile identities independent of Scholar labels", %{
    analysis: analysis,
    config: config,
    dataset: dataset
  } do
    assert {:ok, original} = BLSConformanceReport.build(dataset, analysis, config)

    remapped = %{
      analysis
      | observations:
          Enum.map(analysis.observations, fn observation ->
            %{observation | cluster: 2 - observation.cluster}
          end)
    }

    assert {:ok, remapped_report} = BLSConformanceReport.build(dataset, remapped, config)

    assert remapped_report["descriptive_output"] == original["descriptive_output"]
    refute Jason.encode!(original["descriptive_output"]) =~ "source_cluster"
  end

  test "rejects analysis settings or observations that diverge from the contract", %{
    analysis: analysis,
    config: config,
    dataset: dataset
  } do
    assert {:error, {:analysis_configuration_mismatch, _details}} =
             BLSConformanceReport.build(dataset, %{analysis | tolerance: 0.5}, config)

    [first | rest] = analysis.observations
    changed_first = %{first | unemployment_rate: first.unemployment_rate + 1.0}

    assert {:error, :analysis_observations_mismatch} =
             BLSConformanceReport.build(
               dataset,
               %{analysis | observations: [changed_first | rest]},
               config
             )
  end
  ...
  defp with_value_conditions(dataset) do
    preliminary_months = %{
      "CUUR0000SA0" => MapSet.new([~D[2025-07-01], ~D[2026-06-01]]),
